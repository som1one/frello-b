...
ТОЛЬКО JSON, без markdown/пояснений/приветствий.`;
  ...
  preparePlan({
    ...
    const planJsonInstruction = `\n\nОтвет должен быть РОВНО в формате:\n«Ваша суточная норма калорий для достижения вашей цели: ${targetCalories || '[итоговое выбранное число калорий в день]'} ккал. План на [количество] дней:»\n[JSON-массив дней]\nГДЕ JSON строго по схеме: каждый день — объект с полем "meals", а "meals" — массив объектов с полями: type, recipeName, calories, portionSize.\nЛюбой другой текст (приветствия, описания, советы, batch cooking, расчёты) — ОШИБКА. Если отсутствуют calories или portionSize у любого приёма, это ОШИБКА. При ошибке пересчитай и верни строго по формату без лишнего текста.
