...
ТОЛЬКО JSON, без markdown/пояснений/приветствий.`;
  ...
  preparePlan({
    ...
    const planJsonInstruction = `\n\nВерни ТОЛЬКО фразу «Ваша суточная норма калорий для достижения вашей цели: ${targetCalories || '[итоговое выбранное число калорий в день]'} ккал. План на [количество] дней:» и JSON-массив дней, где каждый день - объект с полем "meals". Каждый "meals" - массив объектов с полями: type, recipeName, calories, portionSize. Любой ответ, который НЕ начинается с «Ваша суточная норма калорий...» и далее JSON, считается ОШИБКОЙ — пересчитай и верни строго по инструкции. Если отсутствуют calories или portionSize у любого приёма пищи, это ОШИБКА — пересчитай и верни корректные числа. НИКАКИХ вступлений/пояснений/списков — после заголовка сразу дни с приёмами (Завтрак/Обед/Ужин и т.д.) в формате «Завтрак: Блюдо (XXX ккал, YYY г)».
...
ПРАВИЛА:
– КАЛОРИЙНОСТЬ И ГРАММЫ ДОЛЖНЫ БЫТЬ У КАЖДОГО ПРИЁМА ПИЩИ БЕЗ ИСКЛЮЧЕНИЙ.
- ЕСЛИ нет чисел (calories/portionSize) — ответ считается НЕДЕЙСТВИТЕЛЬНЫМ, пересчитай и выдай корректные числа.
- Поля "calories" и "portionSize" ОБЯЗАТЕЛЬНЫ и НЕ МОГУТ быть 0, null или отсутствовать.
- "calories" — целое число на БЛЮДО; "portionSize" — целое число граммов на БЛЮДО (оценивать, если точной массы нет).
...
- Поле "portionSize" - ОБЯЗАТЕЛЬНОЕ целое число в граммах (НЕ 0).
