...
export class AiPrepareService {
  ...
  async prepareSimpleMessage({
    ...
    // Проверяем, содержит ли запрос ключевые слова для планов или рецептов
    const planKeywords = [
      "план питания", "план на день", "план на неделю", "план на",
      "расписание питания", "рацион на неделю", "рацион на день", "рацион на",
      "расписание еды", "питание на неделю", "питание на день", "питание на",
      "другой план", "новый план", "еще один план", "составь план",
      "сделай план", "придумай план", "меню на", "меню на неделю", "меню на день",
      "давай другой", "хочу другой", "сделай другой", "другой"
    ];
    
    const recipeKeywords = [
      "рецепт", "как приготовить", "приготовь", "приготовление",
      "пошаговый рецепт", "ингредиенты", "способ приготовления",
      "как сделать", "кулинария", "готовим", "как сварить",
      "как испечь", "как пожарить", "как запечь", "ужин", "обед", "завтрак"
    ];

    const lowerContent = content.toLowerCase();
    const hasPlanKeywords = planKeywords.some(keyword => lowerContent.includes(keyword));
    const hasRecipeKeywords = recipeKeywords.some(keyword => lowerContent.includes(keyword));

    // Если есть ключевые слова - используем полный промпт, иначе - упрощенный
    let systemMessageContent: string;
    if (hasPlanKeywords || hasRecipeKeywords) {
      // Полный промпт
      const now = new Date();
      ...
      const instructionWithDate = FRELLO_INSTRUCTION.replace('{{CURRENT_DATE}}', currentDate);
      systemMessageContent = `${baseMessage} ${instructionWithDate}`;
    } else {
      // Упрощенный промпт
      systemMessageContent = `${baseMessage} Ты - Frello, эксперт по питанию и диетологии. НЕ используй **, ***, ---, ###, #, _ и другие символы форматирования. Пиши обычным текстом.`;
    }
...
- Поле "portionSize" — ОБЯЗАТЕЛЬНОЕ целое число в граммах, представляющее вес порции блюда.Оцени приблизительно, если точная граммовка неизвестна, но НЕ оставляй поле пустым.
- КРИТИЧЕСКИ ВАЖНО: Рассчитай точное БЖУ (proteins, fats, carbs) только на одну порцию с указанными калориями. БЖУ должно соответствовать указанным калориям по формуле: калории = белки × 4 + жиры × 9 + углеводы × 4. Учитывай потери веса при термообработке. БЖУ должно быть реалистичным и сбалансированным для указанного количества калорий.
  ${settingsStr}.
- ВАЖНО: Если пользователь просит "другой", "новый" или "разнообразный" план, ты ОБЯЗАН предложить блюда, отличные от тех, что были в предыдущих сообщениях(истории чата).Старайся не повторяться.
