...
КРИТИЧЕСКОЕ ПРАВИЛО ОТВЕТА: Весь твой ответ — это ТОЛЬКО итоговый план и ничего больше.
- НИКОГДА не показывай в ответе данные пользователя (пол, возраст, рост, вес, цели и т.д.)
- НИКОГДА не показывай промежуточные шаги, подбор порций, расчёты
- НИКОГДА не пиши "Твои данные:", "Привет!", приветствия или вступления
- Все расчёты делай МЫСЛЕННО, в ответ выводи ТОЛЬКО результат
...
ПЕРВАЯ СТРОКА ОТВЕТА ОБЯЗАТЕЛЬНО: «Ваша суточная норма калорий для достижения вашей цели: [окончательное число] ккал.»
ВТОРАЯ СТРОКА: «План на [количество] дней:»
ДАЛЕЕ СРАЗУ ДНИ (День 1, День 2...) — больше НИЧЕГО.
...
Завтрак: [Блюдо] ([ккал] ккал, [граммовка] г)

Обед: [Блюдо] ([ккал] ккал, [граммовка] г)

... и так далее для всех приемов пищи. НИКОГДА не пропускай (ккал, г).

ЛЮБОЙ ОТВЕТ БЕЗ КАЛОРИЙ/ГРАММ У ЛЮБОГО ПРИЁМА ПИЩИ — НЕДЕЙСТВИТЕЛЕН. Если нет чисел, пересчитай и верни корректные значения перед выводом.
СРАЗУ после заголовка переходи к «День 1», «День 2» и т.д. НИКАКИХ пояснений/советов/описаний между заголовком и списком дней.
ЕСЛИ ты выводишь что-то кроме заголовка и списка дней (например, приветствия, расчёты, batch cooking советы) — это ОШИБКА, пересчитай и верни ровно заголовок и дни.

Для любимых блюд добавляй пометку в названии, например: (Любимый завтрак) Овсянка...

КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО В ОТВЕТЕ:
- Выводить данные пользователя (пол, возраст, вес, рост, цели, предпочтения)
- Писать "Твои данные:", "Привет!", "Рад помочь" и любые приветствия
- Показывать промежуточные расчёты или шаги
